...
if [ ! -f ext.tar ]; then
	echo "ext: Not backed up."
else 
	if [ $REST_EXT2 -eq 0 ]; then
		echo "ext: Skipping."
	elif [ ! -d /sddata ]; then
		echo "E: ext partition does not exist"
		echo "ext: Cannot restore."
		ERROR="${ERROR}ext: Attempted to restore non-existing partition.\n"
	else	
		if [ ! -f ext.md5 ]; then
			echo "Partition ext checksum file missing, skipping."		
			...
			ERROR="${ERROR}ext: MD5 checksum file missing.\n"
			
		else
					
			echo -n "ext: Verifying MD5..."
			md5sum -c ext.md5 > /dev/null
			
			if [ $? -eq 1 ]; then
				echo "failed"
				echo "Partition ext checksum mismatch, skipping."
				...
				ERROR="${ERROR}ext: MD5 checksum mismatch.\n"
